...
def get_config():
  config = {
    ...
    'short_max_steps' : -1,   #If set, overrides epochs
    'long_train_epochs' : 3.0,
    'long_max_steps' : -1,   #If set, overrides epochs
    
    'short_warmup_steps' : 0, #Linear warmup steps
    'long_warmup_steps' : 0,

    'eval_steps' : None, #Log, save and eval every ... steps
  ...
  return StaticAttrDict(config)class StaticAttrDict:
  def __init__(self, d):
    self.d = d
